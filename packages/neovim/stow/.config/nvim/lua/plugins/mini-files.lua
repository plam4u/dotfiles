return {
  ...
  keys = {
    ...
    },
    {
      "gp",
      function()
        vim.g.mf_state = vim.g.mf_state or "disabled"
        local state = vim.g.mf_state
        if state == "disabled" then
          state = "narrow"
          MiniFiles.refresh({ windows = { preview = true, width_preview = 25 } })
        elseif state == "narrow" then
          state = "wide"
          MiniFiles.refresh({ windows = { preview = true, width_preview = 70 } })
        elseif state == "wide" then
          state = "disabled"
          MiniFiles.refresh({ windows = { preview = false } })
          MiniFiles.trim_right()
        end
        vim.g.mf_state = state
      end,
      ft = "minifiles",
