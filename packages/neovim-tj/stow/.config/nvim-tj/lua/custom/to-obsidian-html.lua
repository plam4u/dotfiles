...
-- TODO:
--      - Put user-select: none on various highlights (Diagnostic* for example)
...
local function try_query_terminal_color(color)
  ...
  local au = vim.api.nvim_create_autocmd("TermResponse", {
    ...
    callback = function(args)
      hex = "#" .. table.concat { args.data:match "\027%]%d+;%d*;?rgb:(%w%w)%w%w/(%w%w)%w%w/(%w%w)%w%w" }
    end,
  })
...
local function styletable_insert_conceal(state, start_row, start_col, end_row, end_col, conceal_text, hl_group)
...
local function styletable_diff(state)
  ...
  for row = 1, state.buflen do
    ...
    if filler ~= 0 then
      ...
      table.insert(style_line.virt_lines, { { fill:rep(state.width), register_hl(state, "DiffDelete") } })
    end
...
local function _styletable_extmarks_virt_text(state, extmark, namespaces)
  ...
  then
    ...
    do
      ...
      if extmark[4].virt_text_pos == "eol" then
        style_line_insert_virt_text(styletable[row + 1], #vim.fn.getline(row + 1) + 1, { i[1], hlid })
      else
...
local function styletable_statuscolumn(state)
  ...
  if signcolumn ~= "no" then
    ...
    if signcolumn:match "^auto" then
      ...
      local signcount = {}
      for _, extmark in ipairs(vim.api.nvim_buf_get_extmarks(state.bufnr, -1, 0, -1, { details = true })) do
  ...
  for row = 1, state.buflen do
    local status =
      vim.api.nvim_eval_statusline(statuscolumn, { winid = state.winid, use_statuscol_lnum = row, highlights = true })
...
local function styletable_listchars(state)
  ...
  if listchars.eol then
    for row = 1, state.buflen do
      ...
      style_line_insert_overlay_char(style_line, #vim.fn.getline(row) + 1, { listchars.eol, ids.NonText })
    end
  end
  ...
  if listchars.tab and state.tabstop then
    ...
    do
      ...
      style_line_insert_overlay_char(state.style[match.lnum], match.byteidx + 1, { text, ids.Whitespace })
    end
  end
  ...
  if listchars.space then
    for _, match in
      ipairs(vim.fn.matchbufline(state.bufnr, " ", 1, "$") --[[@as (table[])]])
    do
      style_line_insert_overlay_char(state.style[match.lnum], match.byteidx + 1, { listchars.space, ids.Whitespace })
    end
  end
  ...
  if listchars.nbsp then
    for _, match in
      ipairs(vim.fn.matchbufline(state.bufnr, "\226\128\175\\|\194\160", 1, "$") --[[@as (table[])]])
    do
      style_line_insert_overlay_char(state.style[match.lnum], match.byteidx + 1, { listchars.nbsp, ids.Whitespace })
      for i = 2, #match.text do
        style_line_insert_overlay_char(state.style[match.lnum], match.byteidx + i, { "", ids.Whitespace })
      end
    end
  end
end
...
local function extend_style(out, state)
  ...
  -- table.insert(out, ("body {background-color: %s; color: %s}"):format(state.background, state.foreground))
  for hlid, name in pairs(state.highlights_name) do
    ...
    local c = {
      ...
      ["text-decoration-line"] = not vim.tbl_isempty(decor_line) and table.concat(decor_line, " ") or nil,
    ...
    insert_line("." .. highlight_name_to_class_name(name) .. " {" .. table.concat(attrs, "; ") .. "}")
  end
  table.insert(out, "</style>")
end
...
--   table.insert(out, ('<meta name="colorscheme" content="%s"></meta>'):format(html_escape(colorscheme)))
...
local function win_to_html(winid, opt)
  ...
  opt = opt or {}
  local title = table.concat(vim.tbl_map(vim.api.nvim_buf_get_name, vim.tbl_map(vim.api.nvim_win_get_buf, winid)), ",")
...
function M.tohtml(winid, opt)
  ...
  for k, v in pairs(temp_options) do
    current_options[k] = vim.o[k]
    vim.o[k] = v
  end

  opt = opt or {}
  opt.number_lines = false
  local lines = win_to_html(winid or 0, opt)
...
local make_style_from_hl = function(name, hlid)
  --TODO(altermo) use local namespace (instead of global 0)
  local fg = vim.fn.synIDattr(hlid, "fg#")
  local bg = vim.fn.synIDattr(hlid, "bg#")
  local decor_line = {}
  ...
  return ".block-language-neovim "
    .. "."
    .. highlight_name_to_class_name(name)
    .. " {"
    .. table.concat(attrs, "; ")
    .. "}"
end
...
function M.get_styles()
  ...
  local handle_hl = function(name, hl)
    if hl.link then
      return
    end

    local hlID = vim.api.nvim_get_hl_id_by_name(name)
    -- print(vim.inspect(name, hl))
    -- table.insert(lines, name)
    -- table.insert(lines, string.format("hlID: %s", hlID))
    -- vim.list_extend(lines, vim.split(vim.inspect(hl), "\n"))
    table.insert(lines, make_style_from_hl(name, hlID))
  end
  for name, hl in pairs(hls) do
    if handle_hl(name, hl) then
      break
    end
  end
...
-- local lines = M.tohtml(1009)
-- vim.api.nvim_buf_set_lines(117, 0, -1, false, lines)

-- local styles = M.get_styles()
-- vim.api.nvim_buf_set_lines(158, 0, -1, false, styles)

return M

-- Damikiller37: you can change the `--code-background` variable in body in your publish.css to the correct color.
-- btw for styling you can look at the `app.css` within Obsidian or the Publish site for all the CSS it's using. Find it in Dev Tools Sources tab. Near the top you will see all the variables for easier modification
